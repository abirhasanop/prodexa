  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white/80 dark:bg-gray-900/90 backdrop-blur shadow-md">
      <div className="max-w-5xl mx-auto flex items-center justify-between px-4 py-3">
        {/* Logo on the left */}
        <Link to="/" className="text-2xl font-bold text-purple-600 dark:text-pink-400 whitespace-nowrap hover:scale-105 transform transition-all duration-200">
          Prodexa
        </Link>
            ...
            {navLinks.map(link => (
              <Link
                ...
                className={`px-3 py-2 rounded-lg font-medium transition ${
                  location.pathname === link.to && user
                ...
                title={!user ? 'Login required to access this feature' : ''}
              >
                {link.label}
              </Link>
            ))}
